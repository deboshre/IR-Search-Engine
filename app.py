...
def ExtractDisplayContents(docs):
    ...
    for doc in docs:
        ...
        # print(type(temp))
        # print("id", doc["id"])
        dict.append({"screen_name": temp["screen_name"] if 'screen_name' in temp else "", "name": temp["name"] if 'name' in temp else "", "link": doc[
                    "id"] if 'id' in doc else "",  "summary": doc["tweet_text"][0] if 'tweet_text' in doc else ""})
    return dict
...
def per(field):
    ...
    req_url = "http://ec2-18-216-205-125.us-east-2.compute.amazonaws.com:8984/solr/Twoogle/select?json.facet={category:{type:terms,field:" + \
        field + "}}&facet=on&indent=on&q=" + query + "&wt=json"
...
def sentiments():
    ...
    chart = [pp, np, nnp]
    print(chart)
    arr = [{"label": "Positive", "value": chart[0]}, {"label": "Negative",
                                                      "value": chart[1]}, {"label": "Neutral", "value": chart[2]}]
    print(arr)
    return jsonify(arr)
